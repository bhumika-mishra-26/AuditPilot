...
Uses SQLModel to create tables and seed initial data.
"""
    ...
    with Session(engine) as session:
        ...
        for table in tables:
            try:
                ...
                print(f"    {table.__name__:<22} → {count} rows")
            except Exception as e:
                print(f"    {table.__name__:<22} → ERROR: {e}")
...
if __name__ == "__main__":
    # We allow running directly or via flag
    print("🚀 Initializing database (SQLModel)...")
    main()
